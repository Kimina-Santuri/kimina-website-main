...
if (workFilters.length) {
  const workItems = document.querySelectorAll("[data-work-types]");
  const emptyState = document.querySelector(".work-empty");
  workFilters.forEach((filter) => {
    filter.addEventListener("click", () => {
      const selected = filter.dataset.workFilter;
      let visibleCount = 0;
      workFilters.forEach((button) => button.setAttribute("aria-pressed", String(button === filter)));
      workItems.forEach((item) => {
        const matches = selected === "all" || item.dataset.workTypes.split(" ").includes(selected);
        item.hidden = !matches;
